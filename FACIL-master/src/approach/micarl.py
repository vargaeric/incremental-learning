...
class Appr(Inc_Learning_Appr):
    def __init__(self, model, device, nepochs=60, lr=0.5, lr_min=1e-4, lr_factor=3, lr_patience=5, clipgrad=10000,
        ...
        # MiCaRL is expected to be used with exemplars. If needed to be used without exemplars, overwrite here the
        # `_get_optimizer` function with the one in LwF and update the criterion
        have_exemplars = self.exemplars_dataset.max_num_exemplars + self.exemplars_dataset.max_num_exemplars_per_class
        if not have_exemplars:
            warnings.warn("Warning: MiCaRL is expected to use exemplars. Check documentation.")
    ...
    def classify(self, task, features, targets):
        ...
        correlations = torch.nn.functional.cosine_similarity(features, means, dim=1)

        # Task-Aware Multi-Head with weighted exemplars
        num_cls = self.model.task_cls[task]
        offset = self.model.task_offset[task]

        # Apply softmax to convert correlations to weights
        weights = torch.nn.functional.softmax(correlations[:, offset:offset + num_cls], dim=1)

        # Calculate the weighted sum of exemplars for each class
        weighted_exemplars = torch.matmul(weights.unsqueeze(1), means[:, offset:offset + num_cls, :])

        # get distances for all images to the weighted exemplar class means -- weighted nearest prototype
        dists = (features - weighted_exemplars).pow(2).sum(2).squeeze()
    ...
    # Algorithm 2: MiCaRL Incremental Train
    def train_loop(self, t, trn_loader, val_loader):
        ...
        # Algorithm 3: MiCaRL Update Representation
        ...
        # Algorithm 4: MiCaRL ConstructExemplarSet and Algorithm 5: MiCaRL ReduceExemplarSet
